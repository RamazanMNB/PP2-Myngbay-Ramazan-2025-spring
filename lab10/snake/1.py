...
# Загрузка последнего результата игрока
...
# Границы поля
walls = [(0, i) for i in range(GRID_HEIGHT)] + [(GRID_WIDTH , i) for i in range(GRID_HEIGHT)] + \
        [(i, 0) for i in range(GRID_WIDTH)] + [(i, GRID_HEIGHT) for i in range(GRID_WIDTH)]
